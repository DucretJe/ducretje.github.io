...
When we retrieved the credentials, this Jaeger user seemed to be a SysAdmin, meaning he probably own the machine, we never know maybe he can `sudo`?
...
It's an easy box, it's obvious that this `/home/deploy/password-manager` script is our entrypoint to root. The script looks intersting we may find the root's password with it?
...
None of them work.  
Hum... maybe we can find some credentials in the box? I tried to look in the files in $HOME and in `deploy`'s but found nothing of interest.
Upload and run [LinEnum.sh](https://github.com/rebootuser/LinEnum):

We find some creds in `/home/jaeger/ShoppyApp/node_modules/@pm2/io/docker-compose.yml`
It's for DBs, it appears to be test deployment but we never know, are those running?
...
Postgres and Mongo are there, but the nope it doesn't seems to be it.
Most of all I think I should focus on the script, we have sudo to run it, it seems to handle some passwords... No way it's useless!

Tried to extract the strings from it using `strings` but don't find anything interesting.
Pushing a bit, I use `radar2-cutter` to see what's in there, even if I have no clue of how this soft is working (I need to do this module on HTB academy...)
As expected I don't really understand what I see but I see I can extract some strings, let's try again!
I see one intersting line just after `Please enter your master password:`, it's just `Sample` 🤔...

I try to use it with the script and that's it!  
We get another user `deploy:Deploying@pp!`

### Getting root

Once connected to `deploy` I notice we don't use `bash` by default, it's probably a service account.
Still we can't use `sudo` with this user 😢.

I check the groups this user belongs to and see that it belongs to `docker`.
...
Respects to [lockscan](https://app.hackthebox.com/users/217870) for his work, I struggled a lot there but I feel more confident now 🙂
